...
import pandas as pd
from autogluon.common.features.types import R_INT, R_FLOAT, R_DATETIME, R_CATEGORY
...
class MultimodalData:
    ...
    def load_data(self):
        train_data = TabularDataset(os.path.join(self.data_args.data_path, 'train.csv'))
        test_data = TabularDataset(os.path.join(self.data_args.data_path, 'test.csv'))
        val_data = TabularDataset(os.path.join(self.data_args.data_path, 'val.csv')) if self.has_val else None
    ...
    def transform_features(self):
        ...
        self.train_dataset = self.create_dataset(self.train_text_feats, self.train_cat_feats, self.train_num_feats,
                                                 train_labels, None,
                                                 self.label_list)
        self.test_dataset = self.create_dataset(self.test_text_feats, self.test_cat_feats, self.test_num_feats,
                                                test_labels,
                                                None,
                                                self.label_list)
        self.val_dataset = self.create_dataset(self.val_text_feats, self.val_cat_feats, self.val_num_feats, val_labels,
                                               None,
                                               self.label_list) if self.has_val else None
