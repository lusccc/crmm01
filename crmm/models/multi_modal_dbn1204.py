...
class MultiModalBertDBN(PreTrainedModel):
    def __init__(self, config: MultiModalConfig):
        ...
        if self.config.n_modality > 1:
            # TODO change 256 * 2 + 768
            self.joint_dbn = self.create_dbn(n_layer=2, n_visible=256 * 2 + 768, n_hidden=256,
    ...
    def forward(self,
        ...
        elif self.config.n_modality > 1:
            num_dbn_bn_out = self.num_bn(num_dbn_out)
            # make_dot(num_dbn_out, show_attrs=True, ).render("num_dbn_out", directory='./', format="pdf")

            cat_dbn_bn_out = self.cat_bn(cat_dbn_out)

            out_fused = torch.concatenate([num_dbn_bn_out, cat_dbn_bn_out, bert_out_pooled], dim=1)
