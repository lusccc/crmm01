...
class FixGaussianRBM(RBM):
    ...
    def __init__(
        ...
        super(FixGaussianRBM, self).__init__(
            n_visible,
            n_hidden,
            steps,
            learning_rate,
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            # For every batch
            for batch_no, (samples, _) in tqdm(enumerate(batches)):
                if self.normalize:
                    # Normalizing the samples' batch
                    # logger.info(f'!! samples before Normalizing: {samples}')
                    # logger.info(f'!! samples before Normalizing mean: {torch.mean(samples, 0, False)}')
                    # logger.info(f'!! samples before Normalizing std: {torch.std(samples, 0, False)}')
                    samples = (
                            (samples - torch.mean(samples, 0, False))
                            / (torch.std(samples, 0, False) + 1e-6)
                    ).detach()

                    ((samples - torch.mean(samples, 0, True))/ (torch.std(samples, 0, True) + 1e-6)).detach()
                ...
                logger.info(f'## samples[0][0]:{samples[0][0]}')
                logger.info(f'## visible_states[0][0]:{visible_states[0][0]}')
                logger.info(f'## torch.mean(self.energy(samples)): {torch.mean(self.energy(samples))}')
                logger.info(f'## torch.mean(self.energy(visible_states)): {torch.mean(self.energy(visible_states))}')
...
class GaussianReluRBM(FixGaussianRBM):
...
class GaussianSeluRBM(FixGaussianRBM):
...
class VarianceGaussianSeluRBM(RBM):
    """A VarianceGaussianRBM class provides the basic implementation for
    Gaussian-Bernoulli Restricted Boltzmann Machines (without standardization).
    ...
    def __init__(
        ...
        logger.info("Overriding class: RBM -> VarianceGaussianSeluRBM.")

        # Override its parent class
        super(VarianceGaussianSeluRBM, self).__init__(
            n_visible,
            n_hidden,
            steps,
            learning_rate,
