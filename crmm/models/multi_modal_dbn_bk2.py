...
class MultiModalDBN(PreTrainedModel):
    ...
    def fit_dbn(self, dbn, name, feature, epoch):
        ...
        _, samples, targets, _ = dbn.fit(feat_dataset, batch_size=100, epochs=epoch)
        return samples
    ...
    def fit_dbns(self):
        ...
        for name, dbn in self.dbns.items():
            # for each modality
            if name != 'joint':
                ae_emb = self.get_embedding_dataset(name, activation=False)

                # fit dbn of this modality
                dbn_output = self.fit_dbn(dbn, name, ae_emb, [self.config.dbn_train_epoch for _ in range(len(dbn.models))])
                # dbn.save_model(os.path.join(self.config.dbn_model_save_dir, f'{name}_dbn.pt'))
                torch.save(dbn, os.path.join(self.config.dbn_model_save_dir, f'{name}_dbn.pt'))
                ...
                dbn_output = self.fit_dbn(dbn, name, multi_modal_dbn_outputs_fused,
                             [self.config.dbn_train_epoch for _ in range(len(dbn.models))])
                # dbn.save_model(os.path.join(self.config.dbn_model_save_dir, f'{name}_dbn.pt'))
                torch.save(dbn, os.path.join(self.config.dbn_model_save_dir, f'{name}_dbn.pt'))

    def manually_fine_tune_only_dbns(self, epoch=50):
        """
        ...
        for name, dbn in self.dbns.items():
            if name != 'joint':
                entire_emb_dataset = self.get_embedding_dataset(name, activation=False)  # note is already tensor data
        ...
        fc = torch.nn.Linear(self.dbns['joint'].n_hidden[self.dbns['joint'].n_layers - 1], n_class)
        ...
        def perform_forward(batch):
            ...
            for name, dbn in self.dbns.items():
                if name != 'joint':
                    ...
                    multi_modal_dbn_outputs_fused = torch.concatenate(multi_modal_dbn_outputs, dim=1)
                    # multi_modal_dbn_outputs_fused_sp = F.softplus(multi_modal_dbn_outputs_fused)
                    joint_dbn_output = dbn(multi_modal_dbn_outputs_fused)
    ...
    def load_fitted_dbns(self):
        for name, dbn in self.dbns.items():
            dbn = torch.load(os.path.join(self.config.pretrained_dbn_model_dir, f'{name}_dbn.pt'))
