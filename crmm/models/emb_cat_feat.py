...
class CatFeatureExtractor(nn.Module):
    def __init__(self, num_embeddings, embedding_dims, hidden_dim, output_dim, dropout_prob):
        ...
        self.equal_emb_dim = all(element == embedding_dims[0] for element in embedding_dims)
        self.feat_dim = sum(embedding_dims) if not self.equal_emb_dim else embedding_dims[0]
        n_cat_feat = len(self.num_embeddings)
        input_channel = n_cat_feat if self.equal_emb_dim else 1  # 1 for concat to 1 channel vector

        self.res_block1 = ConvResidualBlock(input_channel, 32)
        # self.res_block2 = ConvResidualBlock(32, 64)
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2, padding=0)
        self.fc1 = nn.Linear(int(32 * self.feat_dim / 2), hidden_dim)
        self.dropout = nn.Dropout(p=dropout_prob)
        self.fc2 = nn.Linear(hidden_dim, output_dim)
    ...
    def forward(self, x):
        ...
        if self.equal_emb_dim:
            ...
            embed = torch.cat(x_embed_list, dim=1)
            embed = embed.unsqueeze(1)
