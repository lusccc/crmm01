...
class MultiModalBertDBN(PreTrainedModel):
    ...
    def __init__(self, config: MultiModalConfig):
        ...
        if self.config.use_modality['text']:
            ...
            if self.config.use_rbm_for_text:
        ...
        if self.config.n_modality > 1:
            ...
            if not self.config.use_rbm_for_text:
                if self.config.use_modality['text']:
                    # dbn is used for num and cat feature,
                    # then dbn output will be concatenated with bert out,hence 512+768
                    self.classifier_input_dim = 512 + 768
    ...
    def forward(self,
        ...
        if self.config.use_modality['text']:
            ...
            if self.config.use_rbm_for_text:
                # note: bert_out no need detach! will be detached in models.crmm_gaussian_rbm.CrmmGaussianReluRBM4deep.fit
                text_rbm_out = self.fit_or_forward_rbm(self.text_rbm, 'text', bert_out,
                                                       self.config.rbm_train_epoch)
        ...
        final_fused_out = rbm_outs[-1]  # for single modality, -1 also means the first rbm out

        if self.config.use_modality['text'] and not self.config.use_rbm_for_text:
            final_fused_out = torch.concatenate([final_fused_out, bert_out], dim=1)

        if not self.config.pretrain:
            loss, logits, classifier_layer_outputs = hf_loss_func(final_fused_out,
                                                                  self.classifier,
                                                                  labels,
                                                                  self.tabular_config.num_labels,
                                                                  None)
