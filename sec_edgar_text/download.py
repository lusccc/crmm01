...
class EdgarCrawler(object):
    ...
    def download_filings(self, company_description, edgar_search_string,
        ...
        for i, index_url in enumerate(filings_links):
            ...
            filings_metadata.append(filing_metadata)
        ...
        # my_sec_file = f'{company_description}_{end_date}.txt'
        # my_sec_folder = os.path.join(args.storage, 'sec_aggr')
        # if not os.path.exists(my_sec_folder):
        #     os.mkdir(my_sec_folder)
        # with open(os.path.join(my_sec_folder, my_sec_file), 'w') as outfile:
        #     for meta in filings_metadata:
        #         with open(meta.output_file) as infile:
        #             for line in infile:
        #                 outfile.write(line)
