...
class DBN(Model):
    ...
    def __init__(
        ...
        """ modified to expose trainable parameters!"""
    ...
    def fit(
        ...
        mse, pl = [], []
        samples, targets, transform = None, None, None
        try:
            ...
            if i ==0:
                ...
                for ep in range(epochs[i]):
                    ...
                    for step, (samples, y) in enumerate(batches):
                        ...
                        ds = Dataset(samples.detach().cpu().numpy(), y.clone().cpu().numpy(), None, show_log=False)
